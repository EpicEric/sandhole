...
async fn http_addressing_profanities_domain() {
    ...
    // 2. Start SSH client that will be proxied with a profane name
    ...
    assert!(
        regex::Regex::new(r"^[a-z0-9]+\.safe\.tld$")
            .unwrap()
            .is_match(&hostname),
        "hostname should've matched regex"
    );
    assert!(
        !hostname.contains("shitty"),
        "hostname shouldn't contain profanity"
    );
    ...
    assert_eq!(response_body, "Hello from a safe space!");
}
...
impl russh::client::Handler for SshClient {
    ...
    async fn server_channel_open_forwarded_tcpip(
        ...
        let router = Router::new().route("/", get(async || "Hello from a safe space!"));
