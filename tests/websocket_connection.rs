...
/// This test ensures that Secure WebSockets work.
...
async fn websocket_connection() {
    ...
    // 3. Connect to the HTTP port of our proxy with a WebSocket client
        ...
        let router = Router::new().route(
            ...
            any(|ws: WebSocketUpgrade| async move {
                ws.on_upgrade(|mut socket| async move {
                    socket
                        .send(Message::Text("Hello from WSS!".into()))
                        .await
                        .unwrap();
