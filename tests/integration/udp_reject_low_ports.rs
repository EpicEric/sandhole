...
/// This test ensures that remote forwarding is not allowed for low UDP ports.
#[test_log::test(tokio::test(flavor = "multi_thread"))]
async fn udp_reject_low_ports() {
    ...
    assert!(
        session.tcpip_forward("udp.sandhole", 20555).await.is_ok(),
        "should allow binding on high port"
    );
    assert!(
        session.tcpip_forward("udp.sandhole", 1024).await.is_ok(),
        "should allow binding ports greater than or equal to 1024"
    );
    assert!(
        session.tcpip_forward("udp.sandhole", 1023).await.is_err(),
