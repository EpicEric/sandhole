...
/// This test ensures that local forwarding works for alias-only HTTP/2 services.
#[test_log::test(tokio::test(flavor = "multi_thread"))]
async fn alias_http2() {
    ...
    // 3. Connect to the HTTP/2 alias of our proxy
    let key = load_secret_key(
        std::path::PathBuf::from(std::env::var("CARGO_MANIFEST_DIR").unwrap())
            .join("tests/data/private_keys/key1"),
        None,
    ...
    let channel = session
        .channel_open_direct_tcpip("http2.foobar.tld", 80, "my.hostname", 12345)
        .await
