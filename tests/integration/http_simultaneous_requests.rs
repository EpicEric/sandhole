...
/// This test ensures that a long-running HTTP request doesn't block
/// other simultaneous requests.
#[test_log::test(tokio::test(flavor = "multi_thread"))]
async fn http_simultaneous_requests() {
    ...
    session
        .tcpip_forward("speed.foobar.tld", 80)
        .await
    ...
    let request = Request::builder()
        .method("GET")
        .uri("/slow")
        .header(HOST, "speed.foobar.tld")
        .body(http_body_util::Empty::<bytes::Bytes>::new())
        .unwrap();
    let slow_jh = tokio::spawn(async move {
        ...
        assert_eq!(response_body, "Slow");
        jh.abort();
    });

    // 3. Start short-running request in the middle of the long-running one
    ...
    if slow_jh.is_finished() {
        slow_jh.await.expect("slow join handle failed");
        panic!("slow join handle finished first");
    }
    assert!(!slow_jh.is_finished());
    assert_eq!(response.status(), StatusCode::OK);
    let response_body = String::from_utf8(
        response
            .into_body()
...
impl russh::client::Handler for SshClient {
    ...
    async fn server_channel_open_forwarded_tcpip(
        ...
        let router = Router::new().route("/fast", get(async || "Fast")).route(
            "/slow",
            get(async || {
