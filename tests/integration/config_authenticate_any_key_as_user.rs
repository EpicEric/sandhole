...
use russh::{
    ...
    keys::{key::PrivateKeyWithHashAlg, ssh_key::private::Ed25519Keypair},
};
use rustls::{
    RootCertStore,
...
/// This test ensures that `--authenticate-any-key-as-user` lets
/// a random SSH key proxy a service.
#[test_log::test(tokio::test(flavor = "multi_thread"))]
async fn config_authenticate_any_key_as_user() {
    ...
    session
        .tcpip_forward("random.foobar.tld", 80)
        .await
    ...
    assert_eq!(response_body, "Hopefully not a malicious service.");
    jh.abort();
}
...
impl russh::client::Handler for SshClient {
    ...
    async fn server_channel_open_forwarded_tcpip(
        ...
        let router = Router::new().route("/", get(async || "Hopefully not a malicious service."));
