...
pub const MAX_PACKET_SIZE: usize = std::mem::size_of::<u16>() + u16::MAX as usize;
    ...
    async fn create_port_socket(&self, port: u16) -> color_eyre::Result<u16> {
        ...
        let join_handle = DroppableHandle(tokio::spawn(async move {
            ...
            loop {
                ...
                match socket.peek_from(buf.as_mut()).await {
                    Ok((_, address)) => {
                            ...
                            && let Ok(channel) = handler.tunneling_channel(ip, address.port()).await
                        {
                            ...
                            socket = get_udp_socket((listen_address, port)).expect("");
                            tokio::spawn(async move {
                                let udp_read = Arc::new(connected_socket);
                                ...
                                let udp2ssh = async move {
                                    ...
                                    loop {
                                        match udp_read
                                            .recv(&mut buf.as_mut()[std::mem::size_of::<u16>()..])
                                            .await
                                        {
                                            Ok(len) => {
                                                *buf[..std::mem::size_of::<u16>()]
                                                    .as_mut_array()
                                                    .expect("length checked") =
                                                    (len as u16).to_be_bytes();
                                                if let Err(error) = ssh_write
                                                    .write_all(
                                                        &buf[..std::mem::size_of::<u16>() + len],
                                                    )
                                                    .await
                                                {
                                                    #[cfg(not(coverage_nightly))]
                    ...
                    Err(error) => {
                        #[cfg(not(coverage_nightly))]
                        tracing::warn!(%port, %error, "Error reading from UDP port.")
                    }
                }
            }
        }));
